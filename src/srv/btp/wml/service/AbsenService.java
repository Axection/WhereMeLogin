...
public class AbsenService extends AsyncTask<String, Integer, Boolean> {
	...
	public static final String ENTRY_SESSIONID = "sessionID";
	...
	public static String message = "Menghubungi server untuk melakukan presensi sebagai : ";
	...
	protected Boolean doInBackground(String... values) {
		if (values.length == 3){
			Log.v("AbsenService : status", "Valid data! Working...");
			// 0 : username
			...
			int status = postData(id_user,longitude,latitude);
			
			Log.v("AbsenService : status", status + "");
			AbsenService.SessionIDResult = SessionID;
			if (status == 200){
	...
	private int postData(String id_user, String longitude, String latitude) {
	...
	private void getEntityContent(HttpEntity httpEntity){
			...
			try {
				...
				SessionID = jObject.getString(ENTRY_SESSIONID);
				//Urgency alert
				if(SessionID.equals("-1") || SessionID == null){
					isFail = true;
					isGetDataFailed = true;
				}
			} 
